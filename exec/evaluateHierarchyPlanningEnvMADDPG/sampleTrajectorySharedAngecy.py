...
from src.generateAction.imaginedWeSampleAction import PolicyForUncommittedAgent, PolicyForCommittedAgent, GetActionFromJointActionDistribution, \
        SampleIndividualActionGivenIntention, SampleActionOnChangableIntention, SampleActionOnFixedIntention, SampleActionMultiagent
...
class SampleTrjactoriesForConditions:
    ...
    def __call__(self, parameters):
        ...
        blocksIDForWolfObserve = list(range(numSheepInWe + numAgentsInWe, numSheepInWe + numAgentsInWe + numBlocksForWe))
        ...
        observeWolf = lambda state: [observeOneAgentForWolf(agentID)(state) for agentID in range(numAgentsInWe + numSheepInWe)]
        ...
        wolfFileName = "maddpg{}wolves{}sheep{}blocks{}episodes{}stepSheepSpeed{}WolfActCost{}individ{}_agent".format(
            numWolves, numSheepInWe, numBlocks, maxEpisode, maxTimeStep, sheepSpeedMultiplier, costActionRatio, wolfSelfish)
        wolfModelPaths = [os.path.join(dirName, '..', '..', 'data', 'preTrainModel', wolfFileName + str(i)) for i in range(numAgentsInWe)]
        ...
        composeCentralControlPolicy = lambda observe: ComposeCentralControlPolicyByGaussianOnDeterministicAction(reshapeAction, 
                observe, actOneStep, buildGaussian) 
        ...
        getStateThirdPersonPerspective = lambda state, goalId, weIds: getStateOrActionThirdPersonPerspective(state, goalId, weIds, blocksID) # nochange
        policyForCommittedAgentsInInference = PolicyForCommittedAgent(wolvesCentralControlPolicy, softPolicyInInference, getStateThirdPersonPerspective)
        concernedAgentsIds = wolvesID
        calCommittedAgentsPolicyLikelihood = CalCommittedAgentsContinuousPolicyLikelihood(concernedAgentsIds, policyForCommittedAgentsInInference, rationalityBetaInInference)
        ...
        policyForUncommittedAgentsInInference = PolicyForUncommittedAgent(wolvesID, randomPolicy, softPolicyInInference, getStateFirstPersonPerspective) # random policy, returns action distribution
        calUncommittedAgentsPolicyLikelihood = CalUncommittedAgentsPolicyLikelihood(wolvesID, concernedAgentsIds, policyForUncommittedAgentsInInference) # returns 1
        ...
        calJointLikelihood = lambda intention, state, perceivedAction: calCommittedAgentsPolicyLikelihood(intention, state, perceivedAction) * \
                calUncommittedAgentsPolicyLikelihood(intention, state, perceivedAction) # __* 1

        # ------------ wolves intention ------------------------
        intentionSpacesForAllWolves = [tuple(it.product(sheepsID, [tuple(wolvesID)])) for wolfId in wolvesID] # <class 'tuple'>: ((3, (0, 1, 2)), (4, (0, 1, 2)), (5, (0, 1, 2)), (6, (0, 1, 2)))
        print('intentionSpacesForAllWolves', intentionSpacesForAllWolves)
        wolvesIntentionPriors = [{tuple(intention): 1/len(allPossibleIntentionsOneWolf) for intention in allPossibleIntentionsOneWolf} for allPossibleIntentionsOneWolf in intentionSpacesForAllWolves]
        perceptSelfAction = SampleNoisyAction(deviationFor2DAction)
        perceptOtherAction = SampleNoisyAction(deviationFor2DAction)
        perceptAction = PerceptImaginedWeAction(wolvesID, perceptSelfAction, perceptOtherAction) # input self, others action
        ...
        composeCentralControlPolicyForPlanning = lambda observe: ComposeCentralControlPolicyByGaussianOnDeterministicAction(reshapeAction, 
                observe, actOneStep, buildGaussianForPlanning)
        wolvesCentralControlPoliciesForPlanning = [composeCentralControlPolicyForPlanning(observeWolf)(wolfModelsList, numAgentsInWe)]
        ...
        chooseCommittedAction = GetActionFromJointActionDistribution(wolfChooseActionMethod, getSelfActionIDInThirdPersonPerspective)
        ...
        for trajectoryId in range(self.numTrajectories):
            ...
            if sheepConcernSelfOnly:
                ...
                composeSheepPolicy = lambda sheepModel: lambda state: tuple(reshapeAction(actOneStep(sheepModel, observeSheep(state))))
