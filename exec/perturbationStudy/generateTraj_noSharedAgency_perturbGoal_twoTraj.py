...
class SampleTrajectoryWithPerturbationTwoTraj:
    ...
    def __call__(self, sampleAction, samplePerturbedAction):
        ...
        trajectory = []
        trajectoryPerturbed = []
        statePerturbed = state.copy()
        for runningStep in range(self.maxRunningSteps):
            if self.isTerminal(state):
                trajectory.append((state, None, None, None, None, 0))
                break

            statePerturbed, actionPerturbed, nextStatePerturbed, rewardPerturbed = self.forwardOneStep(statePerturbed, samplePerturbedAction)
            state, action, nextState, reward = self.forwardOneStep(state, sampleAction)

            trajectory.append((state, action, nextState, reward))
            trajectoryPerturbed.append((statePerturbed, actionPerturbed, nextStatePerturbed, rewardPerturbed))

            state = nextState
            statePerturbed = nextStatePerturbed

        return trajectory, trajectoryPerturbed
...
class SampleTrjactoriesForConditions:
    ...
    def __call__(self, parameters):
        ...
        sampleTrajectoryWithPerturbation = SampleTrajectoryWithPerturbationTwoTraj(maxRunningStepsToSample, isTerminal, reset, forwardOneStep)
        ...
        for trajectoryId in range(self.numTrajectories):
            ...
            trajectory, trajectoryPerturbed= sampleTrajectoryWithPerturbation(sampleAction, sampleActionPerturbed)
            trajectories.append(trajectory)
            trajectoriesPerturbed.append(trajectoryPerturbed)

        trajectoryFixedParameters = {'maxRunningStepsToSample': maxRunningStepsToSample, "perturbed": 0}
        trajectoryFixedParametersPerturbed = {'maxRunningStepsToSample': maxRunningStepsToSample, "perturbed": 1}

        self.saveTrajectoryByParameters(trajectories, trajectoryFixedParameters, parameters)
        self.saveTrajectoryByParameters(trajectoriesPerturbed, trajectoryFixedParametersPerturbed, parameters)
